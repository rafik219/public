...
class Backupdb:
    def __init__(self, popname=None, idsite=None, logger=None):
        ...
        self.yaml_file = os.path.join(
                ConfigParam.BACKUP_DIR, ConfigParam.YAML_FILE_NAME)
        self.list_backup_file = self.get_list_backup_file()
        self.now = datetime.now().strftime("%Y/%m/%d %H:%M:%S")

    def get_list_backup_file(self):
        # try to get linux os distribution
        lsb_release = ConfigParam.get_linux_release()
    ...
    def copy_file(self, file_path, dest):
        ...
        if os.path.exists(file_path):
            try:
                ...
                print
                "Error: Can't copy file: %s to %s, cause: %s" % (file_path, dest, ex)
    ...
    def start_backup(self):
        ...
        if not os.path.exists(ConfigParam.BACKUP_DIR):
            try:
                ...
                self.logger.info("backup directory : %s created." %
                                 ConfigParam.BACKUP_DIR)
            except OSError, ex:
                self.logger.error("Can't create %s directory, cause: %s" % (
                    ConfigParam.BACKUP_DIR, ex))
                sys.exit(-1)
        else:
            self.logger.info(
                    "backup dir : %s already exist, deleting content" % ConfigParam.BACKUP_DIR)
            for root, dirs, files in os.walk(ConfigParam.BACKUP_DIR):
                for d in dirs:
                    try:
                        ...
                        self.logger.error(
                                "Can't remove %s directory, cause: %s" % (d, ex))
                for f in files:
                    try:
                        ...
                        self.logger.error(
                                "Can't remove %s file, cause: %s" % (f, ex))
            ...
            with open(ConfigParam.DBCONFIG, "r") as fconf:
                for line in fconf.readlines():
                    if line != "\n":
                        ...
                        self.logger.info("idsite: %s, alias: %s, schema: %s" % (
                            idsite, alias_idsite, schema))

                        # db_connection = "%s/%s" % (schema, ConfigParam.DB_PASSWORD)
                        db_connection = "%s/%s@%s" % (
                            schema, ConfigParam.DB_PASSWORD, ConfigParam.get_hostname())
                        ...
                        backup_table_file_name = "%s_%s_%s_TABLES_%s.dmp" % (
                            alias_idsite, idsite, schema, self.now.split(' ')[0].replace('/', ''))
                        list_backup_table = "PARAMETRESAPPLICATION,SCHEMA,MISSION,MAINTENANCEMISSION"
                        log_backup_file_name = "backup_BDD_%s_%s.log" % (
                            idsite, schema)
                        ...
                        try:
                            ...
                            self.logger.info(
                                    "table backup generated with success on: %s" % backup_table_file_name)
                        except Exception, ex:
                            self.logger.error(
        ...
        for dmp_file in glob.glob(ConfigParam.TMP_BACKUP_TABLE_DIR + "/*.dmp"):
            ...
            if dmp_file.endswith("TABLES_" + self.now.split(' ')[0].replace('/', '') + ".dmp"):
                try:
                    ...
                    self.logger.info("moving file : %s -> %s" %
                                     (dmp_file, ConfigParam.BACKUP_DIR))
                    status = "OK"
                except OSError, ex:
                    self.logger.error("Can't move %s -> %s directory, cause: %s" %
                                      (dmp_file, ConfigParam.BACKUP_DIR, ex))

                backup_tables_status.append(
                        {"path": dmp_file, "status": status})
        ...
        for service in list_service:
            if service == 'oracle':
                ...
                if out != "" and int(out) > 0:
                    ...
                    self.logger.error(
                            "unknown service status for '%s' service, my be service is not started" % service)
        ...
        self.copy_status_dict["centos_release"] = ConfigParam.get_linux_release(
        )

        # create yml file using result dictionary
        with open(self.yaml_file, "w") as file_dumper:
            self.logger.info("dumping result on YAML file: %s" %
                             self.yaml_file)
            try:
                ...
                self.logger.error(
                        "when dumping result on: %s file, cause: %s" % (self.yaml_file, ex))
                sys.exit(-2)
    ...
    def upload_to_sftp_server(self):
        ...
        compress_file = os.path.join(os.path.dirname(
                os.path.abspath(ConfigParam.BACKUP_DIR)), compress_file_name)
        archive_file_path = ""

        # supported_format = shutil.get_archive_formats()
        supported_format = [
            ('zip', 'zip'), ('gztar', 'tar.gz'), ('bztar', 'tar.bz2')]

        for compress_format in supported_format:
            try:
                shutil.make_archive(
                        compress_file, compress_format[0], ConfigParam.BACKUP_DIR, logger=self.logger)
                archive_file_path = "%s.%s" % (
                    compress_file, compress_format[1])
                self.logger.info(
                        "compressing end with success : %s" % archive_file_path)
                break
            except Exception, ex:
                self.logger.error(
                        "'%s' archive format not supported, cause: %s" % (compress_format[0], ex))
        ...
        if os.path.exists(archive_file_path) and os.path.getsize(archive_file_path) > 0:
            try:
                ...
                self.logger.info("Purge local backup directory: %s" %
                                 ConfigParam.BACKUP_DIR)
            except shutil.Error, ex:
                self.logger.error("Can't delete %s directory, cause: %s" % (
                    ConfigParam.BACKUP_DIR, ex))
        retry = 0
        # upload compressed file to the repository.
        while retry < 5:
            transport = paramiko.Transport(
                    (ConfigParam.SFTP_SERVER, ConfigParam.SFTP_SERVER_PORT))
            transport.connect(username=ConfigParam.SFTP_SERVER_LOGIN,
                              password=ConfigParam.SFTP_SERVER_PASSWORD)
            ...
            try:
                if new_dir not in sftp.listdir():
                    sftp.mkdir(new_dir)
                    self.logger.info("directory : '%s' created on sftp server : '%s'" % (
                        new_dir, ConfigParam.SFTP_SERVER))
                else:
                    # delete old backup files, and keep at least 3 backup
                    self.logger.info(
                            "try to purge old backup files on the server")
                    for backup_file in sftp.listdir(new_dir):
                        generation_date = backup_file.split(
                                ".")[0].split("_")[3]
                        if int(self.now.split(' ')[0].replace('/', '')) - int(generation_date) >= 3:
                            try:
                                sftp.remove(os.path.join(new_dir, backup_file))
                                self.logger.info(
                                        "old backup '%s' -> deleted from the server" % backup_file)
                            except Exception, ex:
                                self.logger.error(
                                        "unable to deleted %s backup on server, cause: %s" % (backup_file, ex))
            except Exception, ex:
                self.logger.error(
                        "when trying to list directory on sftp server, cause: %s" % ex)
                sys.exit(-4)

            sftp_archive_path = "%s/%s" % (new_dir,
            ...
            try:
                ...
                self.logger.error(
                        "when sending archive to the server, cause: %s" % ex)
                retry += 1
                sftp.close()
                sleep(3)
                continue
            ...
            try:
                self.logger.info(
                        "try to comparing size between file transferred and local file")
                ...
                if abs(local_archive_size - remote_archive_size) not in range(0, 5):
                    self.logger.error(
                            "transfer not properly completed, retry sending archive !!")
                    retry += 1
                    sleep(3)
                    continue
                else:
                    ...
                    try:
                        ...
                        self.logger.info(
                                "deleting local archive file : %s" % archive_file_path)
                    except OSError, ex:
                        self.logger.error(
                ...
                self.logger.error(
                        "we can't comparing archive size, cause: %s" % ex)
                sys.exit(-5)
...
class Restoredb:
    def __init__(self, logger=None, popname=None, idsite=None):
        ...
        self.pop_id = idsite
        self.url_access = "%s/%s_%s" % (ConfigParam.HTTP_SERVER_URL,
                                        self.pop_name, self.pop_id)
        self.last_file_name = "%s_%s_BACKUP_%s.zip" % (
            self.pop_name, self.pop_id, datetime.now().strftime("%Y%m%d"))
    ...
    def send_http_request(self, url):
        ...
        try:
            ...
            manager.add_password(
                    None, self.url_access, ConfigParam.HTTP_SERVER_LOGIN, ConfigParam.HTTP_SERVER_PASSWORD)
            ...
            self.logger.error(
                    "Problem during connection to this url: %s, cause: %s" % (self.url_access, ex))
        return response
    ...
    def download_last_backup(self):
        ...
        if len(files_on_server) != 0:
            ...
            if resp is not None:
                ...
                self.logger.info('Downloading: %s Bytes: %s' %
                                 (full_url_download_file, file_size))
                file_sz_dl = 0
                block_sz = 8192
                while True:
                    ...
                    status = r"%10d [%3.2f%%]" % (
                        file_sz_dl, file_sz_dl * 100 / file_size)
                    self.logger.info(status)

                if os.path.exists(file_download):
                    self.logger.info(
                            "SUCCESS: file %s downloaded !" % file_download)
                    is_download = True
                else:
        ...
        if is_download and file_download != "":
            try:
                ...
                extract_dir = os.path.join(os.path.dirname(
                        os.path.realpath(__file__)), extract_dir)
                ...
                self.logger.error(
                        "ERROR: during extracting %s file, cause: %s" % (file_download, ex))
    ...
    def start_restoration(self, zip_file=None):
        ...
        if is_extract and extract_dir != "":
            backup_config = os.path.join(
                    extract_dir, "backup_config_status.yml")
            if os.path.exists(backup_config):
                with open(backup_config, 'r') as stream:
                    yaml_file = load(stream)
                    current_linux_release = ConfigParam.get_linux_release()
                    ...
                    for service, result in sorted(yaml_file.items()):
                        ...
                        if service not in ("services", "backupdb", "centos_release", "network"):
                            for opt in result:
                                if opt.get('status') == "OK":
                                    ...
                                    source_config_file = os.path.join(
                                            extract_dir, os.path.basename(opt.get('path')))
                                    ...
                                    if os.path.exists(local_config_file):
                                        ...
                                        if current_linux_release == backup_db_os_release:
                                            try:
                                                # rename origin file
                                                os.rename(
                                                        local_config_file, local_config_file + "-orig")
                                                self.logger.info("\t OK : Renaming %s - %s" % (local_config_file,
                                                                                               local_config_file + "-orig"))
                                                # move local file
                                                os.rename(
                                                        source_config_file, local_config_file)
                                                self.logger.info(
                                                        "\t OK : Moving %s -> %s" % (
                                                        source_config_file, local_config_file))
                                            except Exception, ex:
                                            ...
                                            if service in ("dist", "nagios", "dns") or (
                                                    service == "network" and opt.get('path').find("ifcfg-eth0") == -1):
                                                try:
                                                    # rename origin file
                                                    os.rename(
                                                            local_config_file, local_config_file + "-orig")
                                                    ...
                                                    self.logger.error(
                                                            "ERROR : when rename/move %s file, cause: %s" % (
                                                            local_config_file, ex))
                                            elif service == "oracle":
                                                self.logger.info(
                                                        "Not Need to copy listner.ora & tnsnames.ora !!")
                                            elif service == "filesystem":
                                                self.logger.info(
                                                        "Not Need to copy listner.ora & tnsnames.ora !!")
                                            else:
                                                self.logger.info(
                                                        "==> not treated: %s - %s " % (service, opt))
                                    else:
                                        self.logger.error(
                                                "\t KO : File not found %s" % local_config_file)
                                else:
                                    self.logger.info(
                                            "\t No configuration need for : %s" % opt.get('path'))

                        elif service == "backupdb":
                            linux_release = ConfigParam.get_linux_release()
                            dmpdir = "/home/oracle/dmpdir"
                            for res in result:
                                schema_name = os.path.basename(
                                        res.get('path')).split('_')[2]
                                if res.get('status') == 'OK':
                                    # full_path_current_dir = os.getcwd()
                                    full_path_current_dir = os.path.dirname(
                                    ...
                                    self.logger.info(
                                            "-----------------------------------")
                                    dmp_file_location = os.path.join(os.path.dirname(full_path_current_dir),
                                                                     os.path.join(
                                                                             extract_dir,
                                                                             os.path.basename(res.get('path'))))
                                    user_schema = os.path.basename(
                                            res.get('path')).split('_')[2]
                                    oracle_schema_script_dir = os.path.join(full_path_current_dir, os.path.join(
                                            os.path.dirname(extract_dir), "oracle_schema"))
                                    create_user_sh_script = os.path.join(
                                            oracle_schema_script_dir, "create_users10g.sh")
                                    if os.path.exists(create_user_sh_script):
                                        create_user_sh_command = "/bin/bash %s -u %s" % (
                                            create_user_sh_script, user_schema)
                                        ...
                                        dmp_file_location_new_path = os.path.join(
                                                dmpdir, os.path.basename(dmp_file_location))
                                        cmd1 = "mv %s %s" % (dmp_file_location, os.path.join(
                                                dmpdir, dmp_file_location_new_path))
                                        cmd2 = "chown oracle:dba %s" % dmp_file_location_new_path

                                        self.logger.info("Moving : %s to %s" % (
                                            dmp_file_location, dmp_file_location_new_path))
                                        proc = subprocess.Popen(
                                                cmd1, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                                        out, err = proc.communicate()
                                        self.logger.info("out : %s" % out)
                                        self.logger.info("err : %s" % err)

                                        self.logger.info(
                                                "Changing owner to oracle:dba for : %s" % dmp_file_location_new_path)
                                        proc = subprocess.Popen(
                                                cmd2, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                                        out, err = proc.communicate()
                                        self.logger.info("out : %s" % out)
                                        self.logger.info("err : %s" % err)

                                        # command = 'su - oracle -c "imp %s/lyd7ugp FILE=%s COMMIT=y IGNORE=y"' % (user_schema, dmp_file_location)
                                        command = 'su - oracle -c "impdp %s/lyd7ugp@%s DIRECTORY=dmpdir DUMPFILE=%s TABLE_EXISTS_ACTION=REPLACE"' % (
                                            user_schema, ConfigParam.get_hostname(),
                                            os.path.basename(dmp_file_location))
                                        self.logger.info(
                                                "Launch cmd : %s" % command)
                                        proc = subprocess.Popen(
                                        ...
                                        self.logger.error(
                                                "File Not Found !! : %s" % create_user_sh_script)
                                        self.logger.error(
                                                "can't import content of tables: PARAMETRESAPPLICATION, MISSION, SCHEMA !! : %s" % create_user_sh_script)
                            ...
                            for serv, stat in result.iteritems():
                                ...
                                if stat == "OK":
                                    self.logger.info(
                                            "- Starting services '%s':" % serv)
                                    out, err = self.mange_service(
                                            serv, "start")
                                    if out != "":
...
def main():
    ...
    if run == "restore":
        ...
        log_file = os.path.join(os.path.dirname(
                os.path.realpath(__file__)), log_file_name)

        # file_handler = logging.FileHandler("backup_pop.log")
        handler = logging.handlers.RotatingFileHandler(
                log_file, mode='a', maxBytes=2000000, backupCount=2)
